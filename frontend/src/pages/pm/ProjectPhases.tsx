...
const STATUS_CFG: Record<string, { label: string; color: string }> = {
  pending:   { label: 'Pendente',   color: 'bg-gray-50 text-gray-600 border-gray-200' },
  active:    { label: 'Em andamento', color: 'bg-blue-50 text-blue-700 border-blue-200' },
...
const emptyForm = {
  ...
  status: 'pending', start_date: '', end_date: '', order_index: 0,
}
...
export default function ProjectPhases() {
  ...
  const phases: Phase[] = (data?.phases ?? []).sort(
    (a: Phase, b: Phase) => a.order_index - b.order_index
  )

  function openCreate() {
    setForm({ ...emptyForm, order_index: phases.length })
    setSelected(null)
    setDialog('create')
  }
  ...
  async function handleSave() {
    ...
    try {
      const url    = dialog === 'edit'
        ? `/api/pm/projects/${projectId}/phases/${selected!.id}`
        : `/api/pm/projects/${projectId}/phases`
      ...
      toast.success(dialog === 'edit' ? 'Fase atualizada!' : 'Fase criada!')
      qc.invalidateQueries({ queryKey: ['pm-phases', projectId] })
      setDialog(null)
    } catch (e: unknown) {
  ...
  async function reorder(phase: Phase, dir: 'up' | 'down') {
    const idx   = phases.findIndex(p => p.id === phase.id)
    const other = phases[dir === 'up' ? idx - 1 : idx + 1]
    if (!other) return
    await Promise.all([
      fetch(`/api/pm/projects/${projectId}/phases/${phase.id}`, {
        method: 'PATCH',
        headers: { ...headers, 'Content-Type': 'application/json' },
        body: JSON.stringify({ order_index: other.order_index }),
      }),
      fetch(`/api/pm/projects/${projectId}/phases/${other.id}`, {
        method: 'PATCH',
        headers: { ...headers, 'Content-Type': 'application/json' },
        body: JSON.stringify({ order_index: phase.order_index }),
      }),
    ])
    qc.invalidateQueries({ queryKey: ['pm-phases', projectId] })
  }

  async function handleDelete(p: Phase) {
    if (!confirm(`Excluir fase "${p.name}"?`)) return
    try {
      const res = await fetch(`/api/pm/projects/${projectId}/phases/${p.id}`, {
    ...
    <div className="flex flex-col">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold">Fases do Projeto</h2>
            <p className="text-xs text-muted-foreground mt-0.5">
              Defina os marcos e etapas do projeto
            </p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground space-y-2">
            ...
            <Button size="sm" variant="outline" onClick={openCreate}>
              <Plus className="h-3.5 w-3.5 mr-1" /> Criar primeira fase
            </Button>
          </div>
        ) : (
            ...
            <Table>
              <TableHeader>
                <TableRow className="h-8">
                  ...
                  <TableHead className="text-[10px] uppercase tracking-wide py-1.5 px-3">Fase</TableHead>
              ...
              <TableBody>
                {phases.map((p, idx) => {
                  const cfg = STATUS_CFG[p.status] ?? { label: p.status, color: '' }
                  return (
                    <TableRow key={p.id} className="h-9">
                      <TableCell className="py-1.5 px-3">
                        <div className="flex items-center gap-1">
                          ...
                          />
                          <span className="text-xs text-muted-foreground">{p.order_index + 1}</span>
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="text-xs text-muted-foreground py-1.5 px-3 whitespace-nowrap">
                        {p.start_date
                          ? `${new Date(p.start_date).toLocaleDateString('pt-BR')} → ${p.end_date ? new Date(p.end_date).toLocaleDateString('pt-BR') : '?'}`
                      ...
                      <TableCell className="py-1.5 px-3 text-right whitespace-nowrap">
                        ...
                        <Button variant="ghost" size="icon" className="h-6 w-6" disabled={idx === phases.length - 1}
      ...
      <Dialog open={!!dialog} onOpenChange={o => { if (!o) setDialog(null) }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{dialog === 'edit' ? 'Editar Fase' : 'Nova Fase'}</DialogTitle>
          </DialogHeader>
